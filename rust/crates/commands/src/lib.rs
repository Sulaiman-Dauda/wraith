...
const SLASH_COMMAND_SPECS: &[SlashCommandSpec] = &[
    ...
    SlashCommandSpec {
        name: "config",
        aliases: &[],
        summary: "Inspect Wraith config files or merged sections",
    ...
    SlashCommandSpec {
        name: "memory",
        aliases: &[],
        summary: "Inspect loaded Wraith instruction memory files",
        argument_hint: None,
        resume_supported: true,
        category: SlashCommandCategory::Workspace,
    },
    SlashCommandSpec {
        name: "init",
        aliases: &[],
        summary: "Create a starter WRAITH.md for this repo",
        argument_hint: None,
        resume_supported: true,
        category: SlashCommandCategory::Workspace,
    },
...
pub fn render_slash_command_help() -> String {
    let mut lines = vec![
        ...
        "  [resume] = also available via wraith --resume SESSION.json".to_string(),
    ];
...
impl DefinitionSource {
    fn label(self) -> &'static str {
        match self {
            ...
            Self::ProjectWraith => "Project (.wraith)",
            Self::UserCodexHome => "User ($CODEX_HOME)",
            Self::UserCodex => "User (~/.codex)",
            Self::UserWraith => "User (~/.wraith)",
        }
    }
}
...
pub fn handle_commit_push_pr_slash_command(
    ...
    let body_path = write_temp_text_file("wraith-pr-body", "md", request.pr_body.trim())?;
...
fn discover_definition_roots(cwd: &Path, leaf: &str) -> Vec<(DefinitionSource, PathBuf)> {
    ...
    for ancestor in cwd.ancestors() {
        ...
        push_unique_root(
            &mut roots,
            DefinitionSource::ProjectWraith,
            ancestor.join(".wraith").join(leaf),
        );
    }
    ...
    if let Some(home) = env::var_os("HOME") {
        ...
        push_unique_root(
            &mut roots,
            DefinitionSource::UserWraith,
            home.join(".wraith").join(leaf),
        );
    }
...
fn discover_skill_roots(cwd: &Path) -> Vec<SkillRoot> {
    ...
    for ancestor in cwd.ancestors() {
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectWraith,
            ancestor.join(".wraith").join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectWraith,
            ancestor.join(".wraith").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
    }
    ...
    if let Some(home) = env::var_os("HOME") {
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserWraith,
            home.join(".wraith").join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserWraith,
            home.join(".wraith").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
    }
...
fn render_agents_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       wraith agents".to_string(),
        "  Sources          .codex/agents, .wraith/agents, $CODEX_HOME/agents".to_string(),
    ];
...
fn render_skills_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       wraith skills".to_string(),
        "  Sources          .codex/skills, .wraith/skills, legacy /commands".to_string(),
    ];
    if let Some(args) = unexpected {
        lines.push(format!("  Unexpected       {args}"));
    }
...
mod tests {
    ...
    fn init_git_repo(label: &str) -> PathBuf {
        ...
        run_command(&root, "git", &["config", "user.name", "Wraith Tests"]);
        run_command(&root, "git", &["config", "user.email", "wraith@example.com"]);
    ...
    fn write_external_plugin(root: &Path, name: &str, version: &str) {
        fs::create_dir_all(root.join(".wraith-plugin")).expect("manifest dir");
        fs::write(
            root.join(".wraith-plugin").join("plugin.json"),
            format!(
    ...
    fn lists_skills_from_project_and_user_roots() {
        ...
        let project_commands = workspace.join(".wraith").join("commands");
        ...
        let roots = vec![
            ...
            SkillRoot {
                source: DefinitionSource::ProjectWraith,
        ...
        assert!(report.contains("Project (.wraith):"));
    ...
    fn agents_and_skills_usage_support_help_and_unexpected_args() {
        ...
        assert!(agents_help.contains("Direct CLI       wraith agents"));
