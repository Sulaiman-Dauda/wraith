...
fn format_direct_slash_command_error(command: &str, is_unknown: bool) -> String {
    ...
    if is_unknown {
        ...
        lines.push("  Try              Start `wraith` to use interactive slash commands".to_string());
        lines.push(
            "  Tip              Resume-safe commands also work with `wraith --resume SESSION.json ...`"
                .to_string(),
        );
    }
    lines.join("\n")
...
fn default_permission_mode() -> PermissionMode {
    env::var("WRAITH_PERMISSION_MODE")
        .ok()
        .as_deref()
...
fn print_bootstrap_plan() {
    for phase in runtime::BootstrapPlan::wraith_default().phases() {
        println!("- {phase:?}");
    }
}
...
fn run_login() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Starting Wraith OAuth login...");
    ...
    println!("Wraith OAuth login complete.");
    Ok(())
}

fn run_logout() -> Result<(), Box<dyn std::error::Error>> {
    clear_oauth_credentials()?;
    println!("Wraith OAuth credentials cleared.");
    Ok(())
}
...
fn wait_for_oauth_callback(
    ...
    let body = if callback.error.is_some() {
        "Wraith OAuth login failed. You can close this window."
    } else {
        "Wraith OAuth login succeeded. You can close this window."
    };
    let response = format!(
...
impl LiveCli {
    ...
    fn run_commit(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let path = write_temp_text_file("wraith-commit-message.txt", &message)?;
    ...
    fn run_pr(&self, context: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if command_exists("gh") {
            let body_path = write_temp_text_file("wraith-pr-body.md", &body)?;
    ...
    fn run_issue(&self, context: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if command_exists("gh") {
            let body_path = write_temp_text_file("wraith-issue-body.md", &body)?;
            let output = Command::new("gh")
...
fn sessions_dir() -> Result<PathBuf, Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    let path = cwd.join(".wraith").join("sessions");
...
fn render_memory_report() -> Result<String, Box<dyn std::error::Error>> {
  ...
  Instruction files {}",
    ...
    if project_context.instruction_files.is_empty() {
        ...
        lines.push(
            "  No WRAITH instruction files discovered in the current directory ancestry.".to_string(),
        );
    } else {
        lines.push("Discovered files".to_string());
...
fn init_wraith_md() -> Result<String, Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    Ok(initialize_repo(&cwd)?.render())
}

fn run_init() -> Result<(), Box<dyn std::error::Error>> {
    println!("{}", init_wraith_md()?);
    Ok(())
}
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    writeln!(out, "Wraith CLI v{VERSION}")?;
    writeln!(
        out,
    ...
    writeln!(
        out,
        "  wraith                                  Start the interactive REPL"
    )?;
    writeln!(
        out,
        "  wraith \"summarize this repo\"            Run one prompt and exit"
    )?;
    writeln!(
        out,
        "  wraith prompt \"explain src/main.rs\"     Explicit one-shot prompt"
    )?;
    writeln!(
        out,
        "  wraith --resume SESSION.json /status    Inspect a saved session"
    )?;
    writeln!(out)?;
    ...
    writeln!(
        out,
        "  wraith dump-manifests                   Read upstream TS sources and print extracted counts"
    )?;
    writeln!(
        out,
        "  wraith bootstrap-plan                   Print the bootstrap phase skeleton"
    )?;
    writeln!(
        out,
        "  wraith agents                           List configured agents"
    )?;
    writeln!(
        out,
        "  wraith skills                           List installed skills"
    )?;
    writeln!(out, "  wraith system-prompt [--cwd PATH] [--date YYYY-MM-DD]")?;
    writeln!(
        out,
        "  wraith login                            Start the OAuth login flow"
    )?;
    writeln!(
        out,
        "  wraith logout                           Clear saved OAuth credentials"
    )?;
    writeln!(
        out,
        "  wraith init                             Scaffold WRAITH.md + local files"
    )?;
    writeln!(out)?;
    writeln!(out, "Flags")?;
    writeln!(
    ...
    writeln!(out, "  wraith --model opus \"summarize this repo\"")?;
    writeln!(
        out,
        "  wraith --output-format json prompt \"explain src/main.rs\""
    )?;
    writeln!(
        out,
        "  wraith --allowedTools read,glob \"summarize Cargo.toml\""
    )?;
    writeln!(
        out,
        "  wraith --resume session.json /status /diff /export notes.txt"
    )?;
    writeln!(out, "  wraith agents")?;
    writeln!(out, "  wraith /skills")?;
    writeln!(out, "  wraith login")?;
    writeln!(out, "  wraith init")?;
    Ok(())
}
...
mod tests {
    ...
    fn shared_help_uses_resume_annotation_copy() {
        ...
        assert!(help.contains("available via wraith --resume SESSION.json"));
    }
    ...
    fn init_help_mentions_direct_subcommand() {
        ...
        assert!(help.contains("wraith init"));
        assert!(help.contains("wraith agents"));
        assert!(help.contains("wraith skills"));
        assert!(help.contains("wraith /skills"));
    }
 ...
 M src/main.rs",
    ...
    fn init_template_mentions_detected_rust_workspace() {
        let rendered = crate::init::render_init_wraith_md(std::path::Path::new("."));
        assert!(rendered.contains("# WRAITH.md"));
    ...
    fn ultraplan_progress_lines_include_phase_step_and_elapsed_status() {
        let snapshot = InternalPromptProgressState {
            ...
            detail: Some("reading rust/crates/wraith-cli/src/main.rs".to_string()),
            saw_final_text: false,
        };
    ...
    fn describe_tool_progress_summarizes_known_tools() {
        ...
        assert!(
            describe_tool_progress("bash", r#"{"command":"cargo test -p wraith-cli"}"#)
                .contains("cargo test -p wraith-cli")
        );
        assert_eq!(
