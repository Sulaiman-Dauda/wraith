...
const STARTER_WRAITH_JSON: &str = concat!(
    "{\n",
    "  \"permissions\": {\n",
    "    \"defaultMode\": \"dontAsk\"\n",
    "  }\n",
...
const GITIGNORE_COMMENT: &str = "# Wraith local artifacts";
const GITIGNORE_ENTRIES: [&str; 2] = [".wraith/settings.local.json", ".wraith/sessions/"];
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let wraith_dir = cwd.join(".wraith");
    artifacts.push(InitArtifact {
        name: ".wraith/",
        status: ensure_dir(&wraith_dir)?,
    });

    let wraith_json = cwd.join(".wraith.json");
    artifacts.push(InitArtifact {
        name: ".wraith.json",
        status: write_file_if_missing(&wraith_json, STARTER_WRAITH_JSON)?,
    });
    ...
    let wraith_md = cwd.join("WRAITH.md");
    let content = render_init_wraith_md(cwd);
    artifacts.push(InitArtifact {
        name: "WRAITH.md",
        status: write_file_if_missing(&wraith_md, &content)?,
    });
...
pub(crate) fn render_init_wraith_md(cwd: &Path) -> String {
    let detection = detect_repo(cwd);
    let mut lines = vec![
        "# WRAITH.md".to_string(),
        String::new(),
        "This file provides guidance to Wraith (wraith-cli) when working with code in this repository.".to_string(),
        String::new(),
    ];
    ...
    lines.push("- Keep shared defaults in `.wraith.json`; reserve `.wraith/settings.local.json` for machine-local overrides.".to_string());
    lines.push("- Do not overwrite existing `WRAITH.md` content automatically; update it intentionally when repo workflows change.".to_string());
    lines.push(String::new());
...
#[cfg(test)]
mod tests {
    use super::{initialize_repo, render_init_wraith_md};
    use std::fs;
    use std::path::Path;
    ...
    fn temp_dir() -> std::path::PathBuf {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("wraith-init-{nanos}"))
    }
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(rendered.contains(".wraith/           created"));
        assert!(rendered.contains(".wraith.json       created"));
        assert!(rendered.contains(".gitignore       created"));
        assert!(rendered.contains("WRAITH.md          created"));
        assert!(root.join(".wraith").is_dir());
        assert!(root.join(".wraith.json").is_file());
        assert!(root.join("WRAITH.md").is_file());
        assert_eq!(
            fs::read_to_string(root.join(".wraith.json")).expect("read wraith json"),
            concat!(
                "{\n",
                "  \"permissions\": {\n",
                "    \"defaultMode\": \"dontAsk\"\n",
        ...
        assert!(gitignore.contains(".wraith/settings.local.json"));
        assert!(gitignore.contains(".wraith/sessions/"));
        let wraith_md = fs::read_to_string(root.join("WRAITH.md")).expect("read wraith md");
        assert!(wraith_md.contains("Languages: Rust."));
        assert!(wraith_md.contains("cargo clippy --workspace --all-targets -- -D warnings"));
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        fs::write(root.join("WRAITH.md"), "custom guidance\n").expect("write existing wraith md");
        fs::write(root.join(".gitignore"), ".wraith/settings.local.json\n").expect("write gitignore");

        let first = initialize_repo(&root).expect("first init should succeed");
        assert!(first
            .render()
            .contains("WRAITH.md          skipped (already exists)"));
        let second = initialize_repo(&root).expect("second init should succeed");
        let second_rendered = second.render();
        assert!(second_rendered.contains(".wraith/           skipped (already exists)"));
        assert!(second_rendered.contains(".wraith.json       skipped (already exists)"));
        assert!(second_rendered.contains(".gitignore       skipped (already exists)"));
        assert!(second_rendered.contains("WRAITH.md          skipped (already exists)"));
        assert_eq!(
            fs::read_to_string(root.join("WRAITH.md")).expect("read existing wraith md"),
            "custom guidance\n"
        );
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
        assert_eq!(gitignore.matches(".wraith/settings.local.json").count(), 1);
        assert_eq!(gitignore.matches(".wraith/sessions/").count(), 1);
    ...
    fn render_init_template_mentions_detected_python_and_nextjs_markers() {
        ...
        let rendered = render_init_wraith_md(Path::new(&root));
