...
impl EditSession {
    ...
    fn prompt_color(&self, vim_enabled: bool) -> Color {
        if vim_enabled && self.mode == EditorMode::Normal {
            Color::Rgb { r: 255, g: 202, b: 40 } // Amber — vim Normal mode
        } else {
            Color::Rgb { r: 0, g: 229, b: 255 } // Cyan — default
        }
    }
    ...
    fn render(
        ...
        let prompt_color = self.prompt_color(vim_enabled);
        queue!(out, SetForegroundColor(prompt_color), Print(prompt.as_ref()), ResetColor)?;
        write!(out, "{buffer}")?;
