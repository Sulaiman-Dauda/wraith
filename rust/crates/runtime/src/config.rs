...
pub const WRAITH_SETTINGS_SCHEMA_NAME: &str = "SettingsSchema";
...
impl ConfigLoader {
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        let user_legacy_path = self.config_home.parent().map_or_else(
            || PathBuf::from(".wraith.json"),
            |parent| parent.join(".wraith.json"),
        );
        vec![
            ConfigEntry {
            ...
            ConfigEntry {
                ...
                path: self.cwd.join(".wraith.json"),
            },
            ConfigEntry {
                source: ConfigSource::Project,
                path: self.cwd.join(".wraith").join("settings.json"),
            },
            ConfigEntry {
                source: ConfigSource::Local,
                path: self.cwd.join(".wraith").join("settings.local.json"),
            },
        ]
    }
...
pub fn default_config_home() -> PathBuf {
    std::env::var_os("WRAITH_CONFIG_HOME")
        .map(PathBuf::from)
        .or_else(|| std::env::var_os("HOME").map(|home| PathBuf::from(home).join(".wraith")))
        .unwrap_or_else(|| PathBuf::from(".wraith"))
}
...
fn read_optional_json_object(
    ...
    let is_legacy_config = path.file_name().and_then(|name| name.to_str()) == Some(".wraith.json");
...
mod tests {
    ...
    fn rejects_non_object_settings_files() {
        ...
        let home = root.join("home").join(".wraith");
    ...
    #[test]
    fn loads_and_merges_wraith_config_files_by_precedence() {
        let root = temp_dir();
        let cwd = root.join("project");
        let home = root.join("home").join(".wraith");
        fs::create_dir_all(cwd.join(".wraith")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");

        fs::write(
            home.parent().expect("home parent").join(".wraith.json"),
        ...
        assert_eq!(WRAITH_SETTINGS_SCHEMA_NAME, "SettingsSchema");
