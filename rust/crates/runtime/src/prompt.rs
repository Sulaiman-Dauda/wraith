...
fn discover_instruction_files(cwd: &Path) -> std::io::Result<Vec<ContextFile>> {
    ...
    for dir in directories {
        for candidate in [
            dir.join("WRAITH.md"),
            dir.join("WRAITH.local.md"),
            dir.join(".wraith").join("WRAITH.md"),
            dir.join(".wraith").join("instructions.md"),
        ] {
...
fn render_project_context(project_context: &ProjectContext) -> String {
    ...
    if !project_context.instruction_files.is_empty() {
        bullets.push(format!(
            "Wraith instruction files discovered: {}.",
...
fn render_instruction_files(files: &[ContextFile]) -> String {
    let mut sections = vec!["# Wraith instructions".to_string()];
...
fn render_config_section(config: &RuntimeConfig) -> String {
    ...
    if config.loaded_entries().is_empty() {
        lines.extend(prepend_bullets(vec![
            "No Wraith settings files loaded.".to_string()
        ]));
        return lines.join("\n");
    }
...
fn get_simple_intro_section(has_output_style: bool) -> String {
    format!(
        "You are Wraith, an AI coding agent that lives in the terminal. You understand codebases and help developers read, write, and refactor code, run commands, and execute complex multi-step engineering tasks autonomously. {} Use the instructions below and the tools available to you to assist the user.\n\nIMPORTANT: You must NEVER generate or guess URLs for the user unless you are confident that the URLs are for helping the user with programming. You may use URLs provided by the user in their messages or local files.",
        if has_output_style {
            "Follow your \"Output Style\" below, which describes how you should respond to user queries."
        } else {
            "You work with users through a terminal-native conversational interface with a cyberpunk soul."
        }
    )
}
...
mod tests {
    ...
    fn discovers_instruction_files_from_ancestor_chain() {
        ...
        fs::create_dir_all(nested.join(".wraith")).expect("nested wraith dir");
        fs::write(root.join("WRAITH.md"), "root instructions").expect("write root instructions");
        fs::write(root.join("WRAITH.local.md"), "local instructions")
            .expect("write local instructions");
        fs::create_dir_all(root.join("apps")).expect("apps dir");
        fs::create_dir_all(root.join("apps").join(".wraith")).expect("apps wraith dir");
        fs::write(root.join("apps").join("WRAITH.md"), "apps instructions")
            .expect("write apps instructions");
        fs::write(
            root.join("apps").join(".wraith").join("instructions.md"),
            "apps dot wraith instructions",
        )
        .expect("write apps dot wraith instructions");
        fs::write(nested.join(".wraith").join("WRAITH.md"), "nested rules")
            .expect("write nested rules");
        fs::write(
            nested.join(".wraith").join("instructions.md"),
            "nested instructions",
        )
    ...
    fn dedupes_identical_instruction_content_across_scopes() {
        ...
        fs::write(root.join("WRAITH.md"), "same rules\n\n").expect("write root");
        fs::write(nested.join("WRAITH.md"), "same rules\n").expect("write nested");
    ...
    fn displays_context_paths_compactly() {
        assert_eq!(
            display_context_path(Path::new("/tmp/project/.wraith/WRAITH.md")),
            "WRAITH.md"
        );
    }
    ...
    fn discover_with_git_includes_status_snapshot() {
        ...
        fs::write(root.join("WRAITH.md"), "rules").expect("write instructions");
        ...
        assert!(status.contains("?? WRAITH.md"));
    ...
    #[test]
    fn load_system_prompt_reads_wraith_files_and_config() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".wraith")).expect("wraith dir");
        fs::write(root.join("WRAITH.md"), "Project rules").expect("write instructions");
        fs::write(
            root.join(".wraith").join("settings.json"),
        ...
        let original_wraith_home = std::env::var("WRAITH_CONFIG_HOME").ok();
        std::env::set_var("HOME", &root);
        std::env::set_var("WRAITH_CONFIG_HOME", root.join("missing-home"));
...
",
        ...
        if let Some(value) = original_wraith_home {
            std::env::set_var("WRAITH_CONFIG_HOME", value);
        } else {
            std::env::remove_var("WRAITH_CONFIG_HOME");
        }
    ...
    #[test]
    fn renders_wraith_style_sections_with_project_context() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".wraith")).expect("wraith dir");
        fs::write(root.join("WRAITH.md"), "Project rules").expect("write WRAITH.md");
        fs::write(
            root.join(".wraith").join("settings.json"),
            r#"{"permissionMode":"acceptEdits"}"#,
        )
        ...
        assert!(prompt.contains("# Wraith instructions"));
        assert!(prompt.contains("Project rules"));
        assert!(prompt.contains("permissionMode"));
    ...
    #[test]
    fn discovers_dot_wraith_instructions_markdown() {
        let root = temp_dir();
        let nested = root.join("apps").join("api");
        fs::create_dir_all(nested.join(".wraith")).expect("nested wraith dir");
        fs::write(
        ...
        assert!(context
            .instruction_files
            .iter()
            .any(|file| file.path.ends_with(".wraith/instructions.md")));
        assert!(
    ...
    fn renders_instruction_file_metadata() {
        let rendered = render_instruction_files(&[ContextFile {
            path: PathBuf::from("/tmp/project/WRAITH.md"),
            content: "Project rules".to_string(),
        }]);
        assert!(rendered.contains("# Wraith instructions"));
